...
public static class NeighbourhoodFactory
{
    ...
    public static List<Grain> GetNeighboursGrains(Grain[,,] grains, int x, int y, int z, Config config)
    {
        switch (config.Neighbourhood)
        {
            case ENeighbourhood.vonNeumann:
                return VonNeumannGrains(grains, x, y, z, config);
            case ENeighbourhood.Moore:
                return MooreGrains(grains, x, y, z, config);
            default:
                return null;
        }
    }
    ...
    public static List<Grain> VonNeumannGrains(Grain[,,] grains, int x, int y, int z, Config config)
    {
        ...
        if (config.IsMPI && (x == 0 || x == config.SizeX -1))
